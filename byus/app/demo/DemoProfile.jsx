    ...
    <div className="min-h-screen bg-brand-cream pb-16">
      ...
      <div className="mx-auto max-w-4xl px-6 pt-8">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_280px]">
          <section><h2 className="font-display text-2xl font-bold">Recent posts</h2><div className="mt-4 space-y-3">{creator.samples.map((sample, i) => <article key={sample.title} className="rounded-xl border border-brand-ink/10 bg-brand-paper p-5"><p className="text-xs font-bold uppercase tracking-wide text-brand-clay">{i === 1 ? 'Members only' : 'Public post'}</p><h3 className="mt-1 font-display text-lg font-semibold">{sample.title}</h3><p className="mt-2 text-sm text-brand-ink/60">A sample of how this update would appear on a creator’s ByUs page.</p><div className="mt-3 flex items-center gap-4 text-xs font-semibold text-brand-ink/50"><span className="flex items-center gap-1"><span aria-hidden="true">👁</span> {sample.views.toLocaleString()} views</span><span className="flex items-center gap-1 text-[#A6432E]"><span aria-hidden="true">♥</span> {sample.likes.toLocaleString()} likes</span></div></article>)}</div></section>
