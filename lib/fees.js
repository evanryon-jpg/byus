...
// 2. Platform milestone bonus -- ByUs's own BEST SINGLE MONTH of fee income yet (summed
//    across every creator for whichever calendar month was highest, using the rate
//    actually applied to each invoice) unlocks a further reduction for EVERY creator at
//    once, in `reduction_points`, each time that peak crosses one of the thresholds
//    seeded in `platform_milestones`. Peak-month, not a running lifetime total, so the
//    number actually reflects the platform's current scale rather than just its age --
//    and since creator_earnings rows are never deleted, the peak can only ever go up, so
//    a milestone still can never un-cross. This is applied on top of #1 at
